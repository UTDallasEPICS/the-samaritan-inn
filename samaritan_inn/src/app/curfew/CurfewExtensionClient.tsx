...
export default function CurfewExtensionClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (
      !formData.caseWorker || formData.caseWorker === 'Select Case Worker' ||
      !formData.startDate || !formData.startTime ||
      !formData.endDate || !formData.endTime ||
      !formData.reason || !formData.signature
    ) {
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fc]">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center h-14">
            ...
            <div className="flex items-center space-x-0">
              ...
              <a
                href="/profile"
                className="px-4 py-2 text-white text-sm hover:bg-[#002b80] rounded-md ml-1"
              >
                Profile
              </a>
              <button
      ...
      <div className="max-w-5xl mx-auto px-8 py-8">
        ...
        {showError && (
          <div className="mb-6 bg-red-50 border border-red-300 rounded-md p-3 flex items-center space-x-2 shadow-sm">
            <X className="w-5 h-5 text-red-600" />
            <span className="text-sm text-red-700 font-medium">{showError}</span>
          </div>
        )}

        {/* FORM */}
        <form onSubmit={handleSubmit} className="mb-8 bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="startTime" placeholder="7:00 PM" value={formData.startTime} onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#00bcd4]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input type="text" name="endTime" placeholder="9:00 PM" value={formData.endTime} onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#00bcd4]" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" name="reason" placeholder="e.g., Medical appointment, Job interview" value={formData.reason}
                onChange={handleInputChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#00bcd4]" />
            </div>
            ...
            <div>
              ...
              <input type="text" name="signature" placeholder="Type your full name" value={formData.signature}
                onChange={handleInputChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#00bcd4]" />
            </div>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={loading}
                className={`px-5 py-2 rounded-md text-sm font-medium transition-all ${loading
                  ? "bg-gray-400 cursor-not-allowed"
                  : "bg-[#00bcd4] text-[#002b80] hover:bg-[#26c6da]"
                  }`}
              >
