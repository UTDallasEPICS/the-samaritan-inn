...
export default function MyEventsPage() {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fetchSlots(e.target.value, selectedCalendar.ownerId);
  };

  const handleStartClick = (slot: { start: string; end: string; label: string }) => {
    setSelectedStart(prev => prev?.start === slot.start ? null : slot);
    setSelectedDuration(null);
    setError('');
  };
  ...
  const isDurationAvailable = (minutes: number) => {
    ...
    return needed.length === minutes / 30;
  };

  const handleDurationClick = (minutes: number) => {
    if (!isDurationAvailable(minutes)) {
      ...
      <div className="flex-grow bg-gray-100 p-4">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/2 p-6 bg-white shadow-md rounded-md self-start">
            ...
            {loadingSlots && <p className="mt-4 text-sm text-gray-500">Loading available slots...</p>}
        ...
        <div className="shrink-0 flex flex-col items-end gap-2">
          ...
          <button
            ...
            className={`text-xs font-medium px-3 py-1 rounded transition ${
              past ? 'bg-gray-200 text-gray-700 hover:bg-gray-300' : 'bg-red-50 text-red-700 hover:bg-red-100'
